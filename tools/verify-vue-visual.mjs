// Real-pixel check for @halftone-ui/vue. Mirrors tools/verify-react-visual.mjs: the jsdom harness
// (verify-vue.mjs) proves the adapter LIFECYCLE; this proves it actually RENDERS. It esbuild-bundles
// Vue + the adapter into one IIFE (vue is bundled IN here, not external — this is a standalone page,
// no node_modules at runtime), inlines it into a self-contained page, mounts every primitive in real
// headless Chromium, and asserts every canvas carries ink — then writes a screenshot to eyeball.
//
// Run: node tools/verify-vue-visual.mjs   (writes tools/.verify-vue-visual.png)
...
// ---- the demo app (bundled with Vue into an IIFE) ------------------------------------------------
const app = `
  import { createApp, h, ref } from 'vue';
  import { HalftoneProvider, Surface, Text, Image, Button, Meter, Card, BarChart, LineChart } from ${JSON.stringify(vueIndex)};
  ...
  // go BLANK, not keep showing the old image (the stale-content regression). Two instances cover
  // both paths: a BROKEN src (onerror) and REMOVING the src (empty falsy early-return).
  const SwapImage = {
    props: { box: String, btn: String, to: String },
    setup(props) {
      const swapped = ref(false);
      return () => h('div', { 'data-box': props.box }, [
        h('button', { id: props.btn, onClick: () => { swapped.value = true; } }, 'swap'),
        h(Image, { src: swapped.value ? props.to : imgSrc, screen: 'stipple', color: 'blue', h: 120 }),
      ]);
    },
  };
  ...
  const App = {
    setup() {
      return () => h(HalftoneProvider, { mode: 'dark' }, () => h('div', { style: { display: 'grid', gap: '24px', padding: '24px', width: '360px' } }, [
        h('div', { 'data-box': 'surface' },
          h(Surface, { field: gradient, screen: 'stipple', h: 120, color: 'blue' })),
        h('div', { 'data-box': 'text' },
          h(Text, { text: 'HALFTONE UI', screen: 'stipple' })),
        h('div', { 'data-box': 'image' },
          h(Image, { src: imgSrc, screen: 'stipple' })),
        h('div', { 'data-box': 'button' },
          h(Button, { color: 'blue', screen: 'stipple',
            style: { border: '0', padding: '12px 20px', color: '#0B0C10', font: '600 15px system-ui', cursor: 'pointer' } },
            () => 'Publish')),
        h('div', { 'data-box': 'meter' },
          h(Meter, { value: 0.66, color: 'blue', screen: 'stipple', h: 16 })),
        h('div', { 'data-box': 'card', style: { minHeight: '64px' } },
          h(Card, { color: 'blue', screen: 'stipple',
            style: { padding: '16px', color: '#F2EFE6', font: '15px system-ui' } },
            () => h('h3', { style: { margin: 0 } }, 'Plate registration'))),
        h('div', { 'data-box': 'bar' },
          h(BarChart, { data: [4, 9, 6, 11, 7], caption: 'Impressions by week', color: 'blue', screen: 'stipple', h: 120 })),
        h('div', { 'data-box': 'line' },
          h(LineChart, { data: [3, 6, 4, 9, 7, 12], area: true, caption: 'Ink-up over time', color: 'blue', screen: 'stipple', h: 120 })),
        h('div', { 'data-box': 'imgalpha' },
          h(Image, { src: imgAlphaSrc, screen: 'stipple', color: 'blue', h: 120 })),
        h(SwapImage, { box: 'swapbroken', btn: 'swap-broken', to: BROKEN }),
        h(SwapImage, { box: 'swapempty', btn: 'swap-empty', to: '' }),
      ]));
    },
  };
...
const built = await esbuild.build({
  absWorkingDir: ROOT,
  stdin: { contents: app, resolveDir: HERE, loader: 'js', sourcefile: 'demo.js' },
  bundle: true, format: 'iife', target: 'es2020', charset: 'utf8',
  define: {
  ...
  write: false,
  nodePaths: [path.join(HERE, 'node_modules')], // vue lives in tools/, not at ROOT
});
ok('demo bundles (Vue + adapter -> one IIFE)', built.outputFiles.length === 1);
...
const htmlPath = path.join(HERE, '.verify-vue-visual.html');
...
// wait until all eleven canvases have painted ink (Image is async on its data-URI load)
try {
...
const inks = await page.evaluate(() => [...document.querySelectorAll('canvas')].map((cv) => {
...
const pngPath = path.join(HERE, '.verify-vue-visual.png');
